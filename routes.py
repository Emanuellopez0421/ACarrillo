...
def detalle_recomendacion(recomendacion_id):
    ...
    try:
        cursor.execute("""
            SELECT r.titulo, r.descripcion, r.imagen_url, v.vuelo_id, v.llegada, v.fecha_salida, 
                   v.hora_salida, v.fecha_llegada, v.hora_llegada, v.precio
            FROM recomendaciones r
            JOIN vueloandm v ON r.vuelo_id = v.vuelo_id
...
def boleto_recomendacion(reserva_id):
    ...
    try:
        cursor.execute("""
            SELECT rr.reserva_id, u.nombre AS cliente, v.salida, v.llegada, 
                   v.fecha_salida, v.hora_salida, v.fecha_llegada, v.hora_llegada, 
                   v.modo_vuelo, v.precio, rr.asiento_id
            FROM reservas_recomendaciones_nueva rr
            JOIN usuario u ON rr.usuario_id = u.id
        ...
        flash(f"Error al obtener detalles del boleto: {e}", "danger")
        boleto = None
    finally:
        cursor.close()
        connection.close()
...
def mis_reservas_recomendaciones():
    ...
    try:
        cursor.execute("""
            SELECT rr.reserva_id, rec.titulo, v.salida, v.llegada, v.fecha_salida, 
                   v.hora_salida, v.fecha_llegada, v.hora_llegada, v.precio, rr.asiento_id
            FROM reservas_recomendaciones_nueva rr
            JOIN vueloandm v ON rr.vuelo_id = v.vuelo_id
            JOIN recomendaciones rec ON rec.vuelo_id = v.vuelo_id
